...
def loadsnipparticles( groupnum, run=default_run,tag=default_tag,model=default_model,directory=default_dir):
	...
	tot_ang_mom = np.array(E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/Spin"))[fsid]
	stellar_mass = np.array(E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/Mass") * 1e10)[fsid]
	stellar_abundances = np.array( [ E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/SmoothedElementAbundance/Hydrogen")[fsid], E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/SmoothedElementAbundance/Helium")[fsid], E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/SmoothedElementAbundance/Carbon")[fsid], E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/SmoothedElementAbundance/Nitrogen")[fsid], E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/SmoothedElementAbundance/Oxygen")[fsid], E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/SmoothedElementAbundance/Neon")[fsid], E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/SmoothedElementAbundance/Magnesium")[fsid], E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/SmoothedElementAbundance/Silicon")[fsid], E.readArray("SNIP_SUBFIND", sim, tag, "/Subhalo/Stars/SmoothedElementAbundance/Iron")[fsid]]) 
...
def halo(partstack,fofdat,simattributes,groupnum, snip=False):
	...
	if snip == True:
		...
		try:
			...
			print 'WARNING: could not fit scale height for halo '+ str(groupnum)+' ... check disk alignment?'
			popt = [np.nan, np.nan, np.nan]
		...
		gal_time = (1/(simattributes[3]*100))*(3.086e19/3.1536e16)*quad(t_lookback,gal_a,1)[0]
		gal_age = gal_time
		formtimes = np.array(stack[:,10][stack[:,0]==4])
		t_em = np.array([quad(t_lookback, formtime, a_0) for formtime in formtimes])
		t_em = (1/(simattributes[3]*100))*(3.086e19/3.1536e16)*t_em[:,0]
		t_em_err = (1/(simattributes[3]*100))*(3.086e19/3.1536e16)*t_em[:,1]
		age = t_em
		solar_h = 0.706498
		solar_fe = 0.00110322
		solar_fe_h = np.log10(solar_fe/solar_h)
		...
		fof_fe = fofdat[7][8]
		fof_fe_h = np.log10(fof_fe/fof_h)-solar_fe_h
		fof_stellar_mass = fofdat[6]
		agedisk = age[disk]
		agebin = agedisk < 1.0
		z_age = z_disk[agebin]
		mass_age = mass_disk[agebin]
		hist, bins = np.histogram(np.abs(z_age), bins=100, range=(0,0.003),weights=mass_age)
    		centers = (bins[:-1] + bins[1:]) / 2
		try:
    			popt, pcov = curve_fit(exponential, centers, hist, p0=[hist[0],0.002,1])
		except RuntimeError:
			print 'WARNING: could not fit scale height for recent stars in halo '+ str(groupnum)+'... check disk alignment?'
			popt = [np.nan, np.nan, np.nan]
			pcov = np.eye(3)*np.nan
		newstarsscaleheight = popt[1]*1e3
		newstarsheighterr = np.sqrt(np.diag(pcov))[1]*1e3
		fofarray = np.array([groupnum, fof_fe_h, jz_jcdiskratio, r200, scaleheight, newstarsscaleheight, newstarsheighterr, gal_age, fof_stellar_mass, fof_median_binding_energy  ])
		partarray = np.dstack((stack[:,0][stack[:,0] == 4], starpos[:,0], starpos[:,1], starpos[:,2], starvel[:,0], starvel[:,1], starvel[:,2], starmass, np.zeros(len(starpos[:,0])), np.zeros(len(starpos[:,0])), starj_z, starj_c, starjz_jc, age, PIDS, kappa, star_binding_energy, halo_mask, disc_mask))[0]
...
def extractsnipages(run=default_run,model=default_model,directory=default_dir):
	sim = default_dir + run + "/" + model + "/data"
	snip_tags = np.load(work_dir+'code/snip_tags_L0025N0752_RECALIBRATED.npy')
	snip_age = []
	for i in snip_tags:
		h = E.readAttribute("SNIP_PARTDATA", sim, i, "/Header/HubbleParam")
		Omega0 = E.readAttribute("SNIP_PARTDATA", sim, i, "/Header/Omega0")
		OmegaLambda = E.readAttribute("SNIP_PARTDATA", sim, i, "/Header/OmegaLambda")
		OmegaBaryon = E.readAttribute("SNIP_PARTDATA", sim, i, "/Header/OmegaBaryon")
		a_0 = E.readAttribute("SNIP_PARTDATA", sim, i, "/Header/ExpansionFactor")
		def t_lookback(a):
			return a/(np.sqrt(Omega0*a+OmegaLambda*(a**4)))
...
<<<<<<< HEAD
def savehaloarrays(partarray, fofarray, simattributes, directory=work_dir, snip=False):
	if snip==False:
		...
		stack = partarray
		subfolder = 'savedhalos/%s/%s/' %(run, model)
		...
		PIDs = stars_grp.create_dataset('PIDs', data=stack[:,23])
		kappa = stars_grp.create_dataset('Kappa', data=stack[:,24])
	...
	if snip == True:
		run, model,  tag, h, boxsize, masstable, mass_cut, Omega0, OmegaLambda, OmegaBaryon, a_0 = simattributes
		groupnum, fof_fe_h, jz_jcdiskratio, r200, scaleheight, newstarsscaleheight, newstarsheighterr, gal_age, fof_stellar_mass, fof_median_binding_energy = fofarray
		stack = partarray	
		subfolder = 'savedhalos/%s/%s/' %(run, model)
		filename = directory+subfolder+run+'_'+model+'_'+tag+'_FOF'+str(int(groupnum))+'.hdf5'
		ensure_dir(directory+subfolder)
		...
		foffeh = fof_grp.create_dataset('stellar_fe_h', data=fof_fe_h)
		r_200fof = fof_grp.create_dataset('r200', data=r200)
		diskscaleheight = fof_grp.create_dataset('scaleheight', data=scaleheight)
		newstarsscaleh = fof_grp.create_dataset('newstarsscaleheight', data=newstarsscaleheight)
		newstarsheighte = fof_grp.create_dataset('newstarsheighterr', data=newstarsheighterr)
		gal_age = fof_grp.create_dataset('galaxy_age', data=gal_age)
		median_binding_energy = fof_grp.create_dataset('median_binding_energy', data=fof_median_binding_energy)
		...
		star_jc = stars_grp.create_dataset('jc', data=stack[:,11])
		star_jz_jc = stars_grp.create_dataset('jz_jc', data=stack[:,12])
		age = stars_grp.create_dataset('age', data=stack[:,13])
		PIDs = stars_grp.create_dataset('PIDs', data=stack[:,14])
		kappa = stars_grp.create_dataset('Kappa', data=stack[:,15])
		star_binding_energy = stars_grp.create_dataset('binding_energy', data=stack[:,16])
		halo_mask = stars_grp.create_dataset('halo_mask', data=stack[:,17])
		disc_mask = stars_grp.create_dataset('disc_mask', data=stack[:,18])
		f.close()
...
class Halo(object):
	...
	def loadfromfile(self, run,model,tag,directory = work_dir):
		if self.snip == False:
			...
			fof = f['/fofdata']
			st = f['/stardata']
			self.simattributes = [sa['run'].value, sa['model'].value, sa['tag'].value, sa['h'].value, sa['boxsize'].value, sa['masstable'].value, sa['mass_cut'].value, sa['Omega0'].value, sa['OmegaLambda'].value, sa['OmegaBaryon'].value, sa['ExpansionFactor'].value]
			self.fof = np.array([fof['groupnum'].value, fof['stellarmass'].value, fof['stellar_fe_h'].value, fof['low_high_a_fe_ratio'].value, fof['percenthigh_a_fe'].value, fof['ratio_jz_jc_sim1'].value, fof['fe_h_gradient'].value, fof['r200'].value, fof['scalelength'].value, fof['sersicindex'].value, fof['median_binding_energy'].value)
			abunds = st['abundances'][:,:]
			self.particles = np.dstack((st['type'][:], st['position'][:,0], st['position'][:,1], st['position'][:,2], st['velocity'][:,0], st['velocity'][:,1], st['velocity'][:,2], st['mass'][:], st['fe_h'][:], st['a_fe'][:], st['jz'][:], st['jc'][:], st['jz_jc'][:], abunds[:,0], abunds[:,1], abunds[:,2], abunds[:,3], abunds[:,4], abunds[:,5], abunds[:,6], abunds[:,7], abunds[:,8], st['age'][:], st['PIDs'][:], st['Kappa'][:], np.array([bool(i) for i in st['halo_mask'][:]]), np.array([bool(i) for i in st['disc_mask'][:]]) ))[0]
		if self.snip == True:
			subfolder = 'savedhalos/%s/%s/' %(run, model)
			filename = str(directory)+str(subfolder)+str(run)+'_'+str(model)+'_'+str(tag)+'_FOF'+str(int(self.groupnumber))+'.hdf5'
			if not os.path.exists(filename):
			...
			st = f['/stardata']
			self.simattributes = [sa['run'].value, sa['model'].value, sa['tag'].value, sa['h'].value, sa['boxsize'].value, sa['masstable'].value, sa['mass_cut'].value, sa['Omega0'].value, sa['OmegaLambda'].value, sa['OmegaBaryon'].value, sa['ExpansionFactor'].value ]
			self.fof = np.array([fof['groupnum'].value, fof['stellar_fe_h'].value, fof['ratio_jz_jc_sim1'].value, fof['r200'].value, fof['scaleheight'].value, fof['newstarsscaleheight'].value, fof['newstarsheighterr'].value, fof['galaxy_age'].value, fof['stellarmass'].value, fof['median_binding_energy'].value ])
			self.particles = np.dstack((st['type'][:], st['position'][:,0], st['position'][:,1], st['position'][:,2], st['velocity'][:,0], st['velocity'][:,1], st['velocity'][:,2], st['mass'][:], st['fe_h'][:], st['a_fe'][:], st['jz'][:], st['jc'][:], st['jz_jc'][:], st['age'][:], st['PIDs'][:], st['Kappa'][:], st['binding_energy'][:], np.array([bool(i) for i in st['halo_mask'][:]]), np.array([bool(i) for i in st['disc_mask'][:]]) ))[0]		
	...
	def agescaleheight(self, rad_cut = [0.003,0.015], jzjc_cut=[0.8,1.2], save=False, returnarray=False):
		...
		for i in range(0,len(age_bins)-1):
			...
			heighterr.append(perr[1])
    			scale_heights.append(popt[1])
		scale_heights = np.array(scale_heights)*1e3
		heighterr = np.array(heighterr)*1e3
   		plt.errorbar(bin_center, scale_heights, yerr=heighterr, label=r'actual')
		...
		if save != True and returnarray == False:
			plt.show()
		if returnarray==True:
			return bin_center, scale_heights, age_bins
