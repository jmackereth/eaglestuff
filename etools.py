...
def loadparticles(run=default_run,tag=default_tag,model=default_model,directory=default_dir, mass_cut=[7e11,3e12]):
	...
	if run == 'L0050N0752' and model == 'AGNdT8':
	 fsid = fsid[:-1] #Fix a zero subhalos bug
	groupnumber = np.array(E.readArray("SUBFIND" , sim, tag, "/Subhalo/GroupNumber"))[fsid] #sorted(set()) would probably accomplish the same thing here
	CoP = np.array(E.readArray("SUBFIND", sim, tag, "/Subhalo/CentreOfPotential"))[fsid] #FSID used to find properties of centrals
	...
	mass_cut_groupnums = fofarray[1][np.where(((fofarray[8] > mass_cut[0]) & (fofarray[8] < mass_cut[1])))]
...
def halo(partstack,fofdat,simattributes,groupnum):
	...
	form_z = (1/formtimes)-1
	htime = (1/(simattributes[3]*100))*(3.086e19/3.1536e16)
	Omega0 = simattributes[7]
	OmegaLambda = simattributes[8]
	age = htime*np.sqrt((1/(-4*Omega0*(1+form_z)**4-OmegaLambda*(1+form_z)))-(1/(-4*Omega0-OmegaLambda)))
	fofarray = np.array([groupnum, fof_stellar_mass, fof_fe_h, low_high_a_fe, high_total_a_fe, jz_jcdiskratio, fe_h_grad, r200, r_e, sersic_index, m200])
	partarray = np.dstack((stack[:,0][stack[:,0] == 4], starpos[:,0], starpos[:,1], starpos[:,2], starvel[:,0], starvel[:,1], starvel[:,2], starmass, fe_h, a_fe, starj_z, starj_c, starjz_jc, stars_h, stars_he, stars_c, stars_n, stars_o, stars_ne, stars_mg, stars_si, stars_fe, age))[0]
...
def savehaloarrays(partarray, fofarray, simattributes, directory=work_dir):
	...
	groupnum, fof_stellar_mass, fof_fe_h, low_high_a_fe, high_total_a_fe, jz_jcdiskratio, fe_h_grad, r200, r_e, sersic_index, m200 = fofarray
	stack = partarray
	...
	m_200 = fof_grp.create_dataset('m200', data=m200)
...
class Halo(object):
	...
	def loadfromsim(self, simobject):
		...
		self.particles, self.fof = halo(simobject.particlestack, simobject.fofstack, self.simattributes, self.groupnumber)		
	...
	def agescaleheight(self, rad_cut = [0.003,0.015], jzjc_cut=[0.8,1.2], save=False):
		...
		age_bins = np.linspace(min(age_disk), max(age_disk), num=10, endpoint=True)
		scale_heights = []
		bin_center = []
		for i in range(0,len(age_bins)-1):
			age_mask = (age_disk < age_bins[i+1]) & (age_disk > age_bins[i])
   			z_age = z_disk[age_mask]
    			hist, bins = np.histogram(np.abs(z_age), bins=100, range=(0,0.003))
    			centers = (bins[:-1] + bins[1:]) / 2
   		...
   		plt.scatter(bin_center, scale_heights)
		plt.xlim(bin_center[0]-0.2, bin_center[-1]+0.2)
		plt.ylim(scale_heights[0]-0.1*scale_heights[0], scale_heights[-1]+0.05*scale_heights[-1])
		plt.xlabel(r'$Age$ $[Gyr]$')
		plt.ylabel(r'$Scale$ $Height$ $[Mpc]$')
		if save == True:
